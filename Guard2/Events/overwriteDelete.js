...
exports.execute = async (oChannel, nChannel) => {
    let entry = await nChannel.guild.fetchAuditLogs({ type: "CHANNEL_OVERWRITE_DELETE" }).then(udit => udit.entries.first());
    ...
    const mesajknk = new MessageEmbed().setTitle('Wex - Kanal izin silme Koruması').setDescription(`**${entry.executor.tag}** \`(${entry.executor.id})\` tarafından <#${oChannel.id}> isimli kanalın izinleriyle oynamaya çalıştı ve onu jaile attım.`).setTimestamp()
    if (log) {
