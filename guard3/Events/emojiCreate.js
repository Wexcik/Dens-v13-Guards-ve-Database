...
exports.execute = async (emoji) => {
    let entry = await emoji.guild.fetchAuditLogs({ type: "EMOJI_CREATE" }).then(dens => dens.entries.first());
    ...
    await emoji.delete({reason: "Wex Emoji Oluşturma Koruması"}).catch(err => console.log(`Etkinlik: ${exports.conf.event}\nError: ${err}`))
    ...
    const mesajknk = new MessageEmbed().setTitle('Wex - Emoji oluşturma Koruması').setDescription(`**${entry.executor.tag}** \`(${entry.executor.id})\` tarafından emoji oluşturmaya çalıştı ve onu jaile attım.`).setTimestamp()
    if (log) {
