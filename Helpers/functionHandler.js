...
const Guard2 = global.Guard2;
const Guard3 = global.Guard3;
const Guard4 = global.Guard4;
const Guard5 = global.Guard5;
const SafeMember = require('../Schemas/guvenliList')
module.exports = (client) => {


    async function checkExec(kisiID) {
        let uye = client.guilds.cache.get(global.botConfig.GuildID).members.cache.get(kisiID);
        let guvenliler = whitelist.guvenli || [];
        if (!uye || uye.id === client.user.id || global.botConfig.Owners.some( x=> uye.id === x) || uye.id === uye.guild.ownerId || guvenliler.some(g => uye.id === g.slice(1) || uye.roles.cache.has(g.slice(1)))) return true
        else return false;
    }
